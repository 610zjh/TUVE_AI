...
**红线 #12**：[`workspace_human/`](../../workspace_human/) 是 AI 只读区。AI 不许修改其中的人写 PRD 与其他受保护原始资料。
**Red Line #12**: [`workspace_human/`](../../workspace_human/) is AI read-only. AI may not edit the human-written PRDs and other protected source materials stored there.
...
[`workspace_human/`](../../workspace_human/) 现在主要存的是**人独有的、不可被 AI 替代的原始需求与受保护资料**：
[`workspace_human/`](../../workspace_human/) now primarily holds **human-exclusive source requirements and protected materials that AI should not alter**:
...
而 [`meetings/`](../../meetings/) 是单独的协作记录区，用来放会议纪要、ADR、客户案例复盘和研究沉淀；AI 可以协助起草，但不应静默改写已经定稿的发言记录。
Meanwhile, [`meetings/`](../../meetings/) is a separate collaboration-record area for meeting notes, ADRs, customer reviews, and research writeups; AI may help draft there, but should not silently rewrite finalized speech records.
...
2. **受保护原始资料要保真**。白板、手写笔记、原始转写等一旦被 AI 改写，原始上下文就丢了。
   Protected source materials must remain faithful. Once AI rewrites whiteboards, handwritten notes, or raw transcripts, the original context is gone.
...
- ✅ 用模板帮人**新建** PRD——人确认后由人保存进 `workspace_human/` / Help **draft new** PRDs from templates — human confirms and saves into `workspace_human/`
- ✅ 在 [`meetings/`](../../meetings/) 起草新的会议纪要 / ADR / 复盘文档 / Draft new meeting notes / ADRs / retro docs in [`meetings/`](../../meetings/)

不允许的 AI 动作 / What AI may NOT do:
- ❌ 修改已有 PRD 的需求段 / 验收标准段 / Edit requirements or acceptance-criteria sections of existing PRDs
- ❌ 修改 `workspace_human/` 中的原始资料内容 / Edit protected source materials inside `workspace_human/`
...
5. **写一份事故纪要**到 [`meetings/`](../../meetings/)，登记影响范围、客户是否需要通知、未来防范措施 / File an incident note in [`meetings/`](../../meetings/): scope, customer notification needed?, prevention
